...
* **Reporting Outputs:**
  * Generate chronological "By Month" reports. Shipped in Wave 20 as `src/pages/Reports.tsx`, sourced from `src/features/projects/hooks/useProjectReports.ts`. The Board-facing layout keeps task-status counts exclusively in the donut chart legend, then renders full-width Phase Details followed by an uncapped Milestone Details row (completed / overdue / upcoming for the selected month). Milestone rows include the milestone `notes` field so exceptions carry the project manager's explanation. The supervisor-report email payload and renderer mirror that notes field.
