...
* **Template Immutability (resolved Wave 36, release-hardened)**: `public.tasks.cloned_from_task_id uuid REFERENCES public.tasks(id) ON DELETE SET NULL` (migration `docs/db/migrations/2026_04_18_task_template_origin.sql`). Stamped server-side during `clone_project_template`; NULL means "post-instantiation custom addition" (freely deletable). `trg_enforce_template_scaffold_immutability` blocks app-role deletes plus structural/content/protected-template-settings updates on cloned instance scaffold rows, including attempts by project owners, while still allowing runtime workflow fields such as status/completion, assignments, priority, dates, notes, primary resources, lock state, supervisor report delivery, and non-protected project settings. Explicit postgres/service-role maintenance bypass remains available for audited repair work. `TaskDetailsView` mirrors the DB rule by blocking delete attempts for all cloned template-origin rows. `TaskItem` renders a small indigo "T" badge (with a Wave 33 Radix tooltip reading "From template") on every template-origin row.
