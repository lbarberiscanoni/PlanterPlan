...
* **Cloned scaffold immutability (release hardening)**: `clone_project_template` now stamps cloned-root provenance (`settings.spawnedFromTemplate`) and the source `template_version` during the insert path, before DB immutability applies. `trg_enforce_template_scaffold_immutability` then rejects app-role deletes and structural/content/protected-template-settings edits on `origin = 'instance' AND cloned_from_task_id IS NOT NULL` rows, including attempts by project owners. Workflow state and runtime project configuration such as supervisor report delivery remain mutable; service-role/postgres maintenance bypasses are explicit and reserved for audited repair work.
