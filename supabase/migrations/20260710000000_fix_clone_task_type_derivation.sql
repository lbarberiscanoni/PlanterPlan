-- Fix: clone_project_template mis-typed phase/milestone rows on every clone.
--
-- The RPC omits task_type from its bulk INSERT and relies on the set_task_type
-- BEFORE-INSERT trigger, which derives task_type from the ANCESTOR-CHAIN DEPTH
-- (derive_task_type). But in a single multi-row INSERT a child row's trigger can
-- fire before its ancestor rows exist, so the depth walk terminates early (or hits
-- derive_task_type's NULL-depth -> 'task' fallback). The whole cloned tree shifts
-- one level too shallow (phases -> 'task', milestones -> 'phase', ...), so those
-- rows orphan into the /tasks "Other" bucket and render as loose actionable tasks.
-- Insert order is unordered, so each clone corrupts a different subset (the "Other"
-- count drifts run-to-run).
--
-- Fix: after the bulk INSERT, when the full subtree exists, re-derive task_type for
-- every cloned row from its complete ancestry. A task_type-only UPDATE does NOT
-- re-fire trg_set_task_type (scoped to UPDATE OF parent_task_id) nor any
-- date/envelope trigger (scoped to UPDATE OF start_date/due_date/duration/
-- parent_task_id), so it is authoritative and cannot perturb the date roll-up.
CREATE OR REPLACE FUNCTION public.clone_project_template(p_template_id uuid, p_new_parent_id uuid, p_new_origin text, p_user_id uuid, p_title text DEFAULT NULL::text, p_description text DEFAULT NULL::text, p_start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_due_date timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
    ...
    -- Repair task_type after the bulk insert. trg_set_task_type derives each row's
    -- type from its ancestor-chain DEPTH on INSERT, but in this single multi-row
    -- INSERT a child's BEFORE trigger can fire before its ancestor rows exist,
    -- mis-typing phases/milestones (the tree shifts one level shallow -> rows
    -- orphan into the /tasks "Other" bucket and render as loose tasks). Now that
    -- the whole subtree exists, re-derive every cloned row's type from its complete
    -- ancestry. A task_type-only UPDATE does NOT re-fire trg_set_task_type (scoped
    -- to UPDATE OF parent_task_id) nor any date/envelope trigger, so this value is
    -- authoritative and cannot perturb the date roll-up below.
    UPDATE public.tasks t
    SET task_type = public.derive_task_type(t.parent_task_id)
    FROM temp_task_map m
    WHERE t.id = m.new_id
