-- Remove the dead `settings.is_coaching_task` remnant.
--
-- The Coach role + is_coaching_task trigger were dropped in the 2026-05-15 role
-- collapse (20260515000000_role_hierarchy_collapse.sql). Nothing in the app reads
-- `settings.is_coaching_task` any more, but two SECURITY DEFINER functions still
-- carry it:
--   * clone_project_template() preserves it onto instance clones.
--   * enforce_template_scaffold_immutability() guards it in v_protected_setting_keys.
-- This drops both references and strips the key from the 6 rows that still have it.
-- is_strategy_template / project_kind / spawnedFromTemplate / cloned_from_template_version
-- are all LIVE and deliberately untouched.
--
-- Both function bodies below are reproduced verbatim from the live definitions
-- (clone backs 3,000+ prod clones), with ONLY the is_coaching_task bits removed.

-- 1. clone_project_template — drop the is_coaching_task pair from the instance-branch settings.
...
-- 2. enforce_template_scaffold_immutability — drop is_coaching_task from the
--    protected-settings list (rest of the body unchanged from 20260612030000).
...
-- 3. Strip the dead key from the rows that still carry it (6 as of 2026-06-12).
UPDATE public.tasks
SET settings = settings - 'is_coaching_task'
WHERE settings ? 'is_coaching_task';
