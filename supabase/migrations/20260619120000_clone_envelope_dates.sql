-- Align clone_project_template with the bottom-up envelope engine.
--
-- Before: leaf dates were interval-shifted from the template's own dates. But
-- templates carry NULL dates, and the old waterfall trigger re-laid everything
-- post-insert. With the waterfall removed, that left freshly cloned projects
-- with NULL leaf dates. Fix:
--   * copy the new `duration` column,
--   * seed every task's start = project_start + days_from_start (offset),
--   * leave due NULL — the leaf trigger derives due = start + duration and the
--     envelope trigger rolls containers up,
--   * after the bulk insert, touch every leaf once so the per-row triggers
--     recompute due and cascade the MIN/MAX roll-up to all ancestors (the
--     in-statement trigger order during INSERT..SELECT can otherwise leave
--     containers stale).
