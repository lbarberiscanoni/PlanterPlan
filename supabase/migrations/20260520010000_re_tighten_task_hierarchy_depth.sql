-- Re-tighten the task hierarchy depth cap from 10 back to 4. The earlier
-- relax in 20260520000000 was rolled back per product direction: PlanterPlan's
-- canonical shape is exactly Project → Phase → Milestone → Task → Subtask
-- (subtasks terminal). Sub-sub-tasks don't help church-planter / ministry-
-- leader users and add UX complexity.
--
-- derive_task_type already stamps anything at depth ≥ 4 as 'subtask' so the
-- display semantics are unchanged.
...
COMMENT ON FUNCTION public.enforce_task_hierarchy_depth() IS
    'Prevents task parent changes that would exceed the canonical 4-level hierarchy (project → phase → milestone → task → subtask) or create cycles.';
